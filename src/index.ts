...
import {anotar, argumentosCmd, checkIdUser, logger, loggerMeta, mensajeRandom, obtenerFecha, obtenerHora, obtenerNota, setIdUser} from "./funcionesBasicas";
import { PREGUNTA_FECHA, PREGUNTA_HORA, SALUDO, MSGCONFU, MSGSALUDO, MSGHORA, MSGDIA} from "./constantes";

const Telegraf: any = require('telegraf').Telegraf;
const weather: any = require('weather-js')
const Bot = new Telegraf(''); //Ingresa el token
...
Bot.use((ctx: Context, next: any) => {
  ...
  return next().then(() => {
    ...
    let clog: string = ctx.message.text + '\n' + 'Mensaje enviado a: ' + ctx.from.first_name + '\n'
...
Bot.hears(PREGUNTA_HORA, (ctx: Context) => {
...
Bot.hears(PREGUNTA_FECHA, (ctx: Context) => {
  var mensaje = obtenerFecha()
  ctx.reply(mensaje)
  logger(mensaje)
})
...
Bot.command('nota', (ctx: Context) => {
  ...
  if (checkIdUser(ctx.from.id)) {
    ...
    const respuesta: string = 'Tu nota no fue guardada ya que no tienes permiso :('
    ctx.reply(respuesta)
    logger(respuesta)
  }
})
...
Bot.command('clima', (ctx: Context) => {
  if (checkIdUser(ctx.from.id) == true) {
    weather.find({search: 'Mexico City', degreeType: 'C'}, (err: any, result: any) => {
      if(err) console.log(err);
      const clima: any = result[0].current
      let mensaje: string = 'El dia de hoy estamos a ' + clima.temperature +'ºc con ' + clima.skytext + 
        ', humedad del ' + clima.humidity + '% asi como vientos de hasta ' + clima.windspeed + 
...
Bot.on('text', (ctx: Context) => {
  let mensaje = mensajeRandom(MSGCONFU)
  logger(mensaje)
  ctx.reply(mensaje)
})
